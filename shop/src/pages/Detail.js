  ...
  return (
    <>
      <input
        ...
        className="w-[50%] flex items-center text-sm leading-6 text-slate-400 rounded-md ring-1 ring-slate-900/10 shadow-sm py-1.5 pl-2 pr-3 mx-auto mb-5 hover:ring-slate-300 "
      />
      {errorMessage && (
        <div className="mb-4 text-sm h-[26px] mx-auto max-w-2xl leading-6 order-first rounded-full px-2 text-xs font-semibold leading-7 bg-blue-500 text-white">
          {errorMessage}
        </div>
      )}

      <div className="flex flex-wrap">
        <div className="w-full px-4 mb-4 w-1/3">
          <img
            style={{ backgroundImage: "url(" + clothone + ")" }}
        ...
        <div className="w-full my-10 flex flex-col justify-center items-center">
          <ul className="flex w-96 flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 ">
            {tabs.map((tab, index) => (
              <li key={index}>
                <button
